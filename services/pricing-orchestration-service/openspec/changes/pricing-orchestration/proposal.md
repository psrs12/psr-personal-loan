...
- Extend **Application Management** to persist credit report references, pricing offers, offer selection, and hard pull consent.
...
- `pricing-orchestration`: End-to-end orchestration of soft pull initiation, pricing request assembly and submission to the Decision Platform (Pricing Engine), pricing offer persistence, offer presentation, consent capture, hard pull initiation, and final decision routing (approved / declined / referred).
...
- `application-management`: Extended to persist credit report reference, pricing offers list, selected offer, consent record, and expiry metadata. State machine extended with new pricing lifecycle states.

## Impact

- **New service**: `pricing-orchestration-service` — consumes domain events, orchestrates soft pull, pricing, consent, hard pull, and decision flows.
- **Extended service**: `credit-evaluation-service` — adds soft pull and hard pull as separate orchestration paths.
- **Extended service**: `application-service` — new state transitions, new persistence tables for pricing offers and consent.
